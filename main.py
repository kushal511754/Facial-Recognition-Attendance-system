...
firebase_admin.initialize_app(cred,{
    ...
    'storageBucket': "gs://realtimeattendance-644b5.appspot.com"
})
        ...
        if counter != 0:
            ...
            if counter == 1:
                ...
                datetimeObject = datetime.strptime(studentInfo['last_attendance'],
                                                  "%Y-%m-%d %H:%M:%S")
                secondsElpased = (datetime.now()-datetimeObject).total_seconds()
                print(secondsElpased)
            ...
            if modeType != 3:
                ...
                if counter <= 10:
                    cv2.putText(imgBackground, str(studentInfo['total_attendance']), (861, 125),
                                cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (50, 50, 50), 1)

                    cv2.putText(imgBackground, str(studentInfo['subject']), (1006, 550),
                                cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.5, (100, 100, 100), 1)
                    cv2.putText(imgBackground, str(id), (1006, 493),
                                cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.5, (100, 100, 100), 1)
                    cv2.putText(imgBackground, str(studentInfo['batch']), (910, 625),
                                cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.6, (100, 100, 100), 1)
                    cv2.putText(imgBackground, str(studentInfo['year']), (1025, 625),
                                cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.6, (100, 100, 100), 1)
                    cv2.putText(imgBackground, str(studentInfo['starting_year']), (1125, 625),
                                cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.6, (100, 100, 100), 1)

                    (w, h), _ = cv2.getTextSize(studentInfo['name'], cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, 1)
                    offset = (414-w)/2
